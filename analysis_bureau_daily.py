...
# Employee service database (auto-added to sys.path for standalone and dynamic-load contexts)
import sys as _sys
    ...
    try:
        for row in get_sheet_rows(file_path):
            ...
            # --- ANALYSER LES DONNÉES QUOTIDIENNES ---
                ...
                # FIX: Read Tps Dû (col3) and Tps Eff (col4) for accurate hours
                tps_du_val  = row[3].value if len(row) > 3 else None
                tps_eff_val = row[4].value if len(row) > 4 else None

                # VÉRIFIER CONGÉ/ABSENCE/JOUR FERIE
                row_text = (val_0 + " " + str(raw_scan_val)).upper()

                # FIX: Skip all non-worked day types: congé, authorized absence, unjustified absence
                ...
                if 'Date' not in val_0 and 'Heures' not in val_0:
                    ...
                    # FIX: Use Tps Eff for accurate hours worked (already accounts for
                    # automatic lunch deductions applied by the pointage system)
                    tps_eff_hours = parse_tps_eff(tps_eff_val)
                    if tps_eff_hours is not None:
                        hours_worked = round(tps_eff_hours, 2)
                        # Recompute scan count from scan string (still needed for lateness/lunch logic)
                        _, calculated_count, times_list = parse_scan_times(raw_scan_val)
                    else:
                        # Fallback: calculate from scan pairs
                        total_sec = 0
                    ...
                    # FIX: Use Tps Dû for target hours (respects per-employee contract)
                    tps_du_hours = parse_tps_eff(tps_du_val)
                    if tps_du_hours is not None:
                        target_hours = round(tps_du_hours, 2)
                    else:
                        is_sat = val_0.lower().startswith('sa')
                        is_ram = is_ramadan_date(full_date)
                        if is_sat:
                            target_hours = 4.0
                        elif is_ram:
                            target_hours = 7.0
...
def analyze_row(row):
    ...
    if not is_saturday and len(scans) >= 2 and hours_worked > 0:
        ...
        # FIX: Use Ramadan-aware threshold
        hours_threshold = 6.5 if is_ramadan else 7.0
        cond_morning = (t_last <= limit_1400) and (hours_worked < hours_threshold)
...
def process_daily_analysis(input_dir, output_dir):
    ...
    # FIX: Use per-row target_hours (from Tps Dû) instead of hardcoded values
    df['is_under_hours'] = (df['scan_count'] > 0) & (df['hours_worked'] < df['target_hours'])

    # --- GÉNÉRATION DES STATISTIQUES ---
    # Worked-day metrics (lateness, lunch, half-day) come from days with scans only.
    # Absent count is computed separately from the full df because absent records have
    # hours_worked=0 and would be zeroed out if we filtered by hours_worked > 0 first.
    attendance_cols = ['is_late_930', 'is_late_1000', 'is_late_1400', 'no_lunch', 'is_half_day']
    ...
    monthly_stats_weekday = weekday_records.groupby('name')[attendance_cols].sum()
    ...
    monthly_stats_saturday = saturday_records.groupby('name')[attendance_cols].sum()
    ...
    # Absence counts from the full df (not filtered by hours_worked)
    full_weekday  = df[~df['day_str'].str.startswith('Sa')]
    full_saturday = df[df['day_str'].str.startswith('Sa')]
    monthly_stats_weekday['is_absent']  = (
        full_weekday.groupby('name')['is_absent'].sum()
        .reindex(monthly_stats_weekday.index, fill_value=0)
    )
    monthly_stats_saturday['is_absent'] = (
        full_saturday.groupby('name')['is_absent'].sum()
        .reindex(monthly_stats_saturday.index, fill_value=0)
    )
    ...
    try:
        ...
        # --- MISE À JOUR last_seen PAR EMPLOYÉ ---
        try:
